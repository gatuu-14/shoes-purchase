...
from .models import  Shoe, Order, Contact
...
def order(request):
    if request.method == "POST":
        ...
        order = Order(name = billname,email = billemail,phone = billphone,address = billaddress,city=billcity,shoes = shoes,day_of_buying = days,date = date,loc_from = fl,loc_to = tl)
        order.save()
        return redirect('home')
    else:
        print("error")
